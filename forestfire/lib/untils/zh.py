...
def get_path_china(path,time,element,bound):
    '''
    寻找文件，并且获取该文件的预报时间
    :param path:
    :param time:
    ...
    return files
def model_ch(path,time, element, bound):
    '''

    :param file_path:
    :return:
    '''
    files = get_path_china(path, time, element, bound)
    data_ne_all = []
    data_sw_all = []
...
#     ne,sw = model_ch('/public/home/lihf_hx/yyc/森林火险模型/forestfire_train_2023_7_26/read_micaps/预报样例数据/中国',stime,element,10)
#     #东北
#     if ne_df.empty:
#         ne_df = ne
#     else:
